...
public class PeopleController : ControllerBase
{
    ...
    public async Task<IActionResult> AddPerson(Person person)
    {
        try
        {
            ...
            return CreatedAtRoute("GetPerson", new { id = person.Id }, person);  // 201 Created status code + location of the resource http://localhost:3000/api/people/{id} person object in response body
        }
        catch (Exception ex)
        {
            return StatusCode(StatusCodes.Status500InternalServerError, ex.Message); // 500 internal server error + message in response
        }
    ...
    public async Task<IActionResult> GetPeople()
    {
        try
        {
            ...
            return Ok(people);  // 200 Ok status code + person object in response body
        }
        catch (Exception ex)
        {
            return StatusCode(StatusCodes.Status500InternalServerError, ex.Message); // 500 internal server error + message in response
    ...
    public async Task<IActionResult> UpdatePerson(int id, [FromBody]Person person)
    {
        try
        {
            ...
            await _context.SaveChangesAsync();
            return NoContent(); // 204 Created status code
        }
        catch (Exception ex)
        {
            return StatusCode(StatusCodes.Status500InternalServerError, ex.Message); // 500 internal server error + message in response
